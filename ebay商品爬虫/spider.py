...
writer.writerow(('商品地址', 'id', '价格', '标题', '详细描述',
                  '图片', '多属性组合', '多属性主属性', '销量', '日期', '类别'))

def info(url):
    res = requests.get(url)
    href = res.url
    id  = re.findall('/(\d+)\?hash',res.url)
    selector = etree.HTML(res.text)
    # 价格
    price = selector.xpath('//*[@id="prcIsum"]/text()')
    if not price:
        price = selector.xpath('//*[@id="mm-saleDscPrc"]/text()')
    ...
    imgs = ''
    for i in imgs_:
        imgs  = imgs + i.replace('s-l64','s-l500') +'\n'
    ...
    # //*[@id="vi-VR-brumb-lnkLst"]/table/tbody/tr[1]/td/h2/ul/li[5]/a/span
    scnd = re.findall('<span itemprop="name">(.*?)</span>',res.text)[-2]
    try:
    ...
    if label_.count('- Select -') == 2:
        ...
        try:
            ...
            print("---------------------一个异常---------------")
    writer.writerow((href, id, price, title, comment, imgs, label_, label_main,
                     sales_volume, data, scnd))
    print(href, id, price, title, comment, imgs, label_, label_main, sales_volume, data, scnd)
if __name__ == '__main__':
    key = 'msyeny'
    pgn = 18
    # https://www.ebay.com/str/msyeny?_pgn=1&rt=nc
